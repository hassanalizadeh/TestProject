...
export default async (bp: typeof sdk, db: Database, interactionsToTrack: string[]) => {
  ...
  bp.events.registerMiddleware({
    name: 'custom-analytics.incoming',
    direction: 'incoming',
    handler: incomingMiddleware,
    order: 12, // after nlu and qna
    description: 'Tracks incoming messages for CustomAnalytics purposes'
  })

  bp.events.registerMiddleware({
    name: 'custom-analytics.outgoing',
    direction: 'outgoing',
    handler: outgoingMiddleware,
    order: 5,
    description: 'Tracks outgoing messages for CustomAnalytics purposes'
  })
