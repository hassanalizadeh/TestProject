...
interface State {
  ...
  shownSection: string
  disableCustomAnalyticsFetching?: boolean
}
...
const fetchReducer = (state: State, action): State => {
  if (action.type === 'datesSuccess') {
    ...
    return {
      ...state,
      dateRange,
      disableCustomAnalyticsFetching: false
    }
    ...
    return {
      ...state,
      dateRange,
      disableCustomAnalyticsFetching: true
    }
  } else {
...
const defaultChannels = [
  { value: 'all', label: lang.tr('module.custom-analytics.channels.all') },
  { value: 'api', label: lang.tr('module.custom-analytics.channels.api') }
]

const CustomAnalytics: FC<any> = ({ bp }) => {
  ...
  const [state, dispatch] = React.useReducer(fetchReducer, {
    ...
    pageTitle: lang.tr('module.custom-analytics.dashboard'),
  ...
  useEffect(() => {
    if (!state.dateRange?.[0] || !state.dateRange?.[1] || state.disableCustomAnalyticsFetching) {
      return
    }

    // tslint:disable-next-line: no-floating-promises
    fetchCustomAnalytics(state.selectedChannel, state.dateRange).then(metrics => {
    ...
    // tslint:disable-next-line: no-floating-promises
    fetchCustomAnalytics(state.selectedChannel, previousRange).then(metrics => {
      dispatch({ type: 'receivedPreviousRangeMetrics', data: { dateRange: previousRange, metrics } })
    })
  }, [state.dateRange, state.selectedChannel])

  const fetchCustomAnalytics = async (channel: string, dateRange): Promise<MetricEntry[]> => {
    const startDate = moment(dateRange[0]).unix()
    const endDate = moment(dateRange[1]).unix()

    const { data } = await bp.axios.get(`mod/custom-analytics/channel/${channel}`, {
      params: {
        start: startDate,
        end: endDate
      }
    ...
    return (
      <div className={style.metricsContainer}>
        <NumberMetric
          ...
          name={lang.tr('module.custom-analytics.newUsers', { nb: getMetricCount('new_users_count') })}
          value={getNewUsersPercent()}
        />
        <NumberMetric
          diffFromPreviousRange={activeUserCountDiff}
          previousDateRange={state.previousDateRange}
          name={lang.tr('module.custom-analytics.returningUsers', { nb: getMetricCount('active_users_count') })}
          value={getReturningUsers()}
        />
        <TimeSeriesChart
          name={lang.tr('module.custom-analytics.userActivities')}
          data={activeUsers}
  ...
  const renderConversations = () => {
    ...
    return (
      <div className={style.metricsContainer}>
        ...
        <NumberMetric
          name={lang.tr('module.custom-analytics.messageExchanged')}
          value={getAvgMsgPerSessions()}
          iconBottom="chat"
        />
        {isNDU && (
          <NumberMetric
            name={lang.tr('module.custom-analytics.workflowsInitiated')}
            value={getMetricCount('workflow_started_count')}
            className={style.half}
          />
        )}
        <NumberMetric
          name={lang.tr('module.custom-analytics.questionsAsked')}
          value={getMetricCount('msg_sent_qna_count')}
          className={style.half}
        />
        <ItemsList
          name={lang.tr('module.custom-analytics.mostUsedWorkflows')}
          items={getTopItems('enter_flow_count', 'workflow')}
          itemLimit={10}
          className={cx(style.genericMetric, style.half, style.list)}
        />
        <ItemsList
          name={lang.tr('module.custom-analytics.mostAskedQuestions')}
      ...
      <div className={cx(style.metricsContainer, style.fullWidth)}>
        <div className={cx(style.genericMetric, style.quarter)}>
          <div>
            ...
            <h3 className={style.metricName}>{lang.tr('module.custom-analytics.misunderstoodMessages')}</h3>
          </div>
          <div>
        ...
        {isNDU && (
          <Fragment>
            <div className={cx(style.genericMetric, style.quarter, style.list, style.multiple)}>
              <ItemsList
                name={lang.tr('module.custom-analytics.mostFailedWorkflows')}
                items={getTopItems('workflow_failed_count', 'workflow')}
                itemLimit={3}
                className={style.list}
              />
              {/* <ItemsList
                name={lang.tr('module.custom-analytics.mostFailedQuestions')}
            ...
            <RadialMetric
              name={lang.tr('module.custom-analytics.successfulWorkflowCompletions', {
            ...
            />
            <RadialMetric
              name={lang.tr('module.custom-analytics.positiveQnaFeedback', { nb: positiveFeedback })}
  ...
  const shortcuts: IDateRangeShortcut[] = [
    {
      ...
      label: lang.tr('module.custom-analytics.timespan.thisWeek')
    },
    {
      dateRange: [lastWeekStart, lastWeekEnd],
      label: lang.tr('module.custom-analytics.timespan.lastWeek')
    },
    {
      dateRange: [thisMonth, now],
      label: lang.tr('module.custom-analytics.timespan.thisMonth')
    },
    {
      dateRange: [lastMonthStart, lastMonthEnd],
      label: lang.tr('module.custom-analytics.timespan.lastMonth')
    },
    {
      dateRange: [thisYear, now],
      label: lang.tr('module.custom-analytics.timespan.thisYear')
    },
    {
      dateRange: [lastYearStart, lastYearEnd],
      label: lang.tr('module.custom-analytics.timespan.lastYear')
    }
  ]
  ...
  const exportCsv = async () => {
    ...
    link.download = `custom-analytics.csv`
    link.click()
  }
  const exportJson = () => {
    ...
    link.download = `custom-analytics.json`
    link.click()
  }
      ...
      <div className={style.innerWrapper}>
        <div className={style.header}>
          <h1 className={style.pageTitle} onDoubleClick={() => loadJson.current.click()}>
            {lang.tr('module.custom-analytics.title')}
          </h1>
          <div className={style.filters}>
            <BpTooltip content={lang.tr('module.custom-analytics.filterChannels')} position={Position.LEFT}>
            ...
            <Popover>
              <Button icon="calendar" className={style.filterItem}>
                {lang.tr('module.custom-analytics.dateRange')}
              </Button>
              <DateRangePicker
            ...
            <Popover
              content={
                <div style={{ padding: 5 }}>
                  <ButtonGroup>
                    <Button onClick={exportCsv} text={lang.tr('module.custom-analytics.exportCsv')}></Button>
                    <Button onClick={exportJson} text={lang.tr('module.custom-analytics.exportJson')}></Button>
                  </ButtonGroup>
                </div>
              }
              position={Position.BOTTOM}
            >
              <Button className={style.exportButton} icon="export" text={lang.tr('module.custom-analytics.export')}></Button>
            </Popover>
          </div>
        </div>
        <div className={style.sectionsWrapper}>
          <div className={cx(style.section, style.half)}>
            <h2>{lang.tr('module.custom-analytics.engagement')}</h2>
            {renderEngagement()}
          </div>
          <div className={cx(style.section, style.half)}>
            <h2>{lang.tr('module.custom-analytics.conversations')}</h2>
            {renderConversations()}
          </div>
          <div className={style.section}>
            <h2>{lang.tr('module.custom-analytics.handlingAndUnderstanding')}</h2>
